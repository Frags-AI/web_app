...
const BlogPage: React.FC = () => {
  ...
  return (
    <div className="bg-[#050406] text-white min-h-screen">
      <Header />
      ...
      <section className="mt-10 flex flex-col md:flex-row items-center gap-6 px-6">
        <div className="bg-gray-800 w-full md:w-2/3 h-64 rounded-lg" />
        <div className="md:w-1/3">
          ...
          <p className="text-gray-400 mt-2">Sep 23, 2024</p>
        </div>
      </section>
          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded-full ${
                selectedCategory === category ? "bg-gray-600 text-white" : "bg-gray-800 text-gray-400"
              }`}
      ...
      <section className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-6">
        {blogPosts.map((post, index) => (
          <Card key={index} className="bg-gray-900 p-4 rounded-lg">
            <div className="bg-[#1A1A1C]-800 h-40 rounded" />
            <CardContent>
